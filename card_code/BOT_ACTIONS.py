    ...
    def Atack_Bot(self, trump: str, deck: Deck):
        ...
        for element in self.Bot_deck:
            if element['suit'] != trump:
                if element['value'] in Hierarchy and self.check_similar(f"{element['value']}-"
                                                                        f"{lexicon.simbol(element['suit'])}", deck):
            ...
            if len(list(test_mass.keys())) != 0:
                ...
                for elem in self.Bot_deck:
                    if elem == self.Bot_deck[self.comparative_deck.index(ix)]:
                        photo = elem['image']
                deck.field_add(f"{self.Bot_deck[self.comparative_deck.index(ix)]['value']}-"
                               f"{lexicon.simbol(self.Bot_deck[self.comparative_deck.index(ix)]['suit'])}")
